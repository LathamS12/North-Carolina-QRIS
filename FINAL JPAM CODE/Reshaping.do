...
*************************************************************/		
	...
	*************************************
	...
	//Using assumptions to generate initial year
		...
		//Defining the primary sample used in all the analysis
			gen ctr1_samp1 = ERS_yr_1st < .  //All centers w/ERS
			replace ctr1_samp1 = . if family_care ==1
			replace ctr1_samp1 = 0 if agerange=="5-12"
			label var ctr1_samp1 "All centers w/ERS ratings that serve children 0-5"
	...
	**************************************************************	
				...
				foreach x in ctr1 	{
					...
					*******************************************
					if `baseyr' == 2007	{
						...
						forvalues i = 2007/2014	{
							replace `x'_`var'_0 = `var'_`i' 	 if `initial' == `i' & ctr1_samp==1
						}
						...
						//Generate T+1 - T+5 outcomes
							forvalues z = 1/5	{			
								...
								forvalues i = 2007/2014	{
									replace `x'_`var'_`z' = `var'_`i'  if `x'_plus`z' == `i' & `x'_`var'_0 !=. & ctr1_samp==1
					...
					*******************************************
					if `baseyr' == 2009	{
						...
						forvalues z = 2/5	{			
							...
							forvalues i = 2007/2014	{
								replace `x'_`var'_`z' = `var'_`i'  if `x'_plus`z' == `i' & `initial' !=. & ctr1_samp==1
								label var `x'_`var'_`z' "`varlab' in T + `z'"
							} //close i loop			
						} //close z loop
					} //close if baseyr 
		...
		foreach rating in ECERS ITERS SACERS	{ //ERS scores
			assign_yr	avg`rating'	"Average `rating' rating"		2007
			assign_yr	any`rating'	"Any `rating' rating"			2007
			assign_yr	num`rating'	"Number of `rating' ratings"	2007
		...
		assign_yr	avgFCCERS	"Average FCCERS rating" 2007		
		...
		assign_yr 	enroll_03	"Enrollment (0-3 yo)"			2007
		assign_yr	enroll_45	"Enrollment (4-5 yo)"			2007
		assign_yr	enroll_sa	"Enrollment (school age)"		2007
		assign_yr	prop_03		"Prop. 0-3 yo"					2007
		assign_yr	prop_45		"Prop. 4-5 yo"					2007 
		assign_yr	prop_sa		"Prop. school aged"				2007
	...
	//Variables that need to be created after assigning based on treatment year
		...
		forvalues i = 1/5	{
			loc last = `i'-1
			replace ctr1_first_change = 	`i' if (`ERS'_`i' != `ERS'_`last')  & ctr1_first_change ==.		& ctr1_samp==1
			replace ctr1_first_rerate = 	`i' if (`ERS'_`i' != `ERS'_`last') & (`ERS'_`i' !=.)  & ctr1_first_rerate ==. & ctr1_samp==1
		}
	...
	************************************************
		...
		***********************
			...
			//Placebo cut points
			ctrlab 5 5.25	ctr1 avgERS
			ctrlab 6 5.5	ctr1 avgERS
			ctrlab 7 5.75	ctr1 avgERS
			ctrlab 8 6.0	ctr1 avgERS		
