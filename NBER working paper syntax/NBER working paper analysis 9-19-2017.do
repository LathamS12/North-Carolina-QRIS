...
*************************************************************************
	...
	****************
	...
	// Table 4 - Estimates do not include controls
		...
		rdtab "closed noERS_open rerated " 	2	ctr1	ERS_yr_1st	"Attrition RD estimates" //These estimates are split across table A2, A3, A4
	...
	// providers with competition > median rather than >= median. Affected about 40 obs, doesn't change pattern/significance of results at all
		...
		hetero "closed noERS_open rerated"	ctr1  2 "Attrition by sample RD estimates" //These estimates are split across tables A2 A3 and A4
	...
	**********************
			...
			preserve
				...
				//Plot binned outcomes on either side of cut points
					...
					foreach dvstub in three_star four_star	{
						...
						twoway  (scatter `dv'_mean bin if tag `bandwidth', msymbol(circle) msize(small) mcolor(black)) ///	
								(qfit `dv' `fv' if `fv' <0 `bandwidth', lwidth(medthick) lcolor(navy)) ///
								(qfit `dv' `fv' if `fv' >=0 `bandwidth', lwidth(medthick) lcolor(navy)), ///
									leg(off) xline(0)  yscale(range(0 1)) ylab(0(.2)1)	 ///
									xtitle("Average ERS rating centered at 4.5") ytitle("p(`dvtit')")
						
						graph display, ysize(10) xsize(7.5)
						graph export "${path}\Figures/Figure 1 - `dvstub'.png", replace
	...
	//Figure 2a
		...
		program dist
			...
			foreach bin in `bins'	{
				...
				if `c' == 1	{
					...
					hist `fv' if `samp' ==1, start(2) width(`bin') ///
						xline(4.5, lcolor(black)) xlabel(2(.5)7) xtitle("Average ERS rating in baseline year") freq ytitle("Frequency") ///
					...
					hist `fv' if `fv' >=4.0 & `fv' < 5.0 & `samp'==1, start(4.0) width(`bin') ///
					 xline(4.5, lcolor(black)) xlabel(4.0(.25)5.0) xtitle("Average ERS rating in baseline year") freq  ytitle("Frequency") ///
					 note("Bins are of size `bin'")	 nodraw
			...
			graph export "${path}/Figures/Figure 2a.png", replace
	...
	//Figure 2b - Have to add label in manually
		use "${path}\Generated datasets\Full dataset wide", clear
			...
			args samp cut type
				...
				cap drop $densvars
				DCdensity `type'_avgERS_0 if `type'_`samp', breakpoint(`cut') generate($densvars) graphname("${path}\Figures\McCrary_`samp'_`type'_`cutsave'.png")
	...
	use "${path}\Generated datasets\Full dataset wide", clear
						...
						* Plot outcomes separately, then use graph combine
						...
						forvalues i = 0/5	{
							...
							twoway  (scatter `dv'_`i'_mean bin if tag `bandwidth', msymbol(circle) msize(small) mcolor(black)) ///	
										(qfit `dv'_`i' `fv' if `fv' <0 `bandwidth', lwidth(medthick) lcolor(navy)) ///
										(qfit `dv'_`i' `fv' if `fv' >=0 `bandwidth', lwidth(medthick) lcolor(navy)), ///
										leg(off) xline(0)  `ychars' ///
		...
		rdfigs ctr1 three_star	"3+ stars" 	"p(3+ stars)"   fv_2 	samp1 	"yscale(range(.4 1)) ylab(.4(.2)1)" "Figure 3a"
		rdfigs ctr1 four_star	"4+ stars" 	"p(4+ stars)"   fv_2 	samp1 	"yscale(range(0 1)) ylab(0(.2)1)"	"Figure 3b"
						...
						foreach samp in samp1 hicomp lowcomp	{
							...
							* Plot outcomes separately, then use graph combine				
							...
							twoway  (scatter `dv'_mean_`samp' bin_`samp' if tag_`samp' `setband', msymbol(circle) msize(small) mcolor(black)) ///	
										(qfit `dv'_5 `fv' if `fv' <0 `setband' & ctr1_`samp' ==1, lwidth(medthick) lcolor(navy)) ///
										(qfit `dv'_5 `fv' if `fv' >=0 `setband' & ctr1_`samp' ==1, lwidth(medthick) lcolor(navy)), ///
										leg(off) xline(0) xscale(range(-`band' `band')) xlabel(-`band'(`delta')`band') `ychars' ///
		...
		full_hi_low avgERS 		"Average ERS rating"			"ERS rating" 		"yscale(range(4.5 5.5)) ylab(4.5(.5)5.5)" 	"Figure 4a" "Figure 5a"
		
		full_hi_low enroll		"Total enrollment" 				"Enrollment" 		"yscale(range(20 80)) ylab(20(20)80)" 		"Figure 4b" "Figure 5b"
		full_hi_low prop_full	"Proportion of capacity filled"	"Proportion filled"	"yscale(range(.4 1)) ylab(.4(.2)1)" 		"Figure 4c" "Figure 5c"
			
		full_hi_low closed		"Likelihood of closure"			"p(closed)"			"yscale(range(0 1)) ylab(0(.2)1)"			"Figure A2a" "Figure A2b"
	
		//full_hi_low noERS_open  "No ERS rating, still open"  	"p(no ERS)"			"yscale(range(0 1)) ylab(0(.2)1)" 			"a"
